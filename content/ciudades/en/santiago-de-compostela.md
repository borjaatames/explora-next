---
nombre: "Santiago de Compostela"
slug: "santiago-de-compostela"
descripcion: "The end of the Camino: the cathedral, the Obradoiro and a World Heritage stone old town. The spiritual capital of Galicia."
comunidad: "Galicia"
publicada: true
destacada: false
orden: 15
keywords:
  - "santiago de compostela"
  - "travel to santiago"
  - "what to see in santiago de compostela"
  - "santiago travel guide"
  - "camino de santiago"
slugs:
  es: santiago-de-compostela
  en: santiago-de-compostela
atracciones:
  - nombre: "Santiago Cathedral"
    descripcion: "The goal of the Camino and one of Europe's great cathedrals: the Pórtico de la Gloria, the botafumeiro and the tomb of the Apostle. The spiritual heart of the city."
  - nombre: "Obradoiro Square"
    descripcion: "The great square before the cathedral's Baroque façade, framed by the Hostal de los Reyes Católicos and the Pazo de Raxoi. Where pilgrims end their journey."
  - nombre: "Old town"
    descripcion: "The maze of stone streets with arcades, squares and churches, a UNESCO World Heritage Site. One of the best-preserved medieval ensembles in Spain."
  - nombre: "Camino de Santiago"
    descripcion: "The network of pilgrimage routes that converge on the city from across Europe. The final stretch, from Sarria or Monte do Gozo, fills Santiago with walkers."
  - nombre: "Mercado de Abastos"
    descripcion: "The second most visited spot after the cathedral: the great Galician produce market, where you can buy seafood, octopus, cheese and empanada and have it cooked on the spot."
  - nombre: "Alameda Park"
    descripcion: "The city's classic park, with the best view of the cathedral and its towers over the old town, especially at sunset."
chipsFiltros:
  - { label: "Cathedral", tag: "catedral" }
  - { label: "Obradoiro", tag: "obradoiro" }
  - { label: "Old town", tag: "casco-historico" }
  - { label: "Camino de Santiago", tag: "camino" }
  - { label: "Food & drink", tag: "gastronomia" }
---

Santiago de Compostela is the capital of Galicia —around 100,000 people— and one of the world's great pilgrimage destinations for a thousand years: this is where the **Camino de Santiago** ends, before the tomb of the Apostle.

Everything revolves around the **Cathedral** and **Obradoiro Square**, where pilgrims complete their journey. Around it spreads a stone **old town** of arcades and squares, a UNESCO World Heritage Site and one of the best preserved in Europe. Don't miss the **Mercado de Abastos**, the second most visited spot, or the view of the towers from **Alameda Park**.

Santiago is also the gateway to Galicia: the **Rías Baixas**, the **Costa da Morte** and A Coruña are all nearby. The city is seen well in **one or two days**, and it's famous for its student atmosphere and its food (octopus, seafood, tarta de Santiago).

The best time to visit is **May to October**; the Galician climate is rainy, so always bring a layer and an umbrella. In **Holy Years** (when 25 July falls on a Sunday), visitor numbers soar.
