---
nombre: "Cádiz"
slug: "cadiz"
descripcion: "The oldest inhabited city in the West, surrounded by sea on three sides. An old town, urban beaches and the wit of the 'Little Silver Cup'."
comunidad: "Andalusia"
imagen: "/images/actividades/cadiz/ciudades/cadiz-hero.webp"
imagenAlt: "Cádiz watchtower with the old town and the sea behind"
publicada: true
destacada: false
orden: 10
keywords:
  - "cadiz"
  - "travel to cadiz"
  - "what to see in cadiz"
  - "cadiz travel guide"
  - "cadiz tourism"
slugs:
  es: cadiz
  en: cadiz
atracciones:
  - nombre: "Cádiz Cathedral"
    descripcion: "The Baroque and Neoclassical cathedral facing the Atlantic, with its golden dome. Climb the Clock Tower to see the city ringed by sea on every side."
  - nombre: "Torre Tavira and the watchtowers"
    descripcion: "The tallest watchtower in the old town, with a camera obscura. A reminder of 18th-century mercantile Cádiz, when the city monopolised trade with the Americas."
  - nombre: "El Pópulo quarter and old town"
    descripcion: "The medieval core, the oldest part of the city, with its arches, the Roman theatre and charming squares where everyday Cádiz still beats."
  - nombre: "La Caleta beach"
    descripcion: "The urban beach between the castles of San Sebastián and Santa Catalina, the most characterful in Cádiz and a film location. Postcard sunsets."
  - nombre: "Jerez de la Frontera"
    descripcion: "Half an hour away, the home of sherry wine, flamenco and horses: centuries-old bodegas and the Royal Andalusian School of Equestrian Art."
  - nombre: "Tarifa and the beaches"
    descripcion: "To the south, the great windswept beaches of the Costa de la Luz, a kitesurfing paradise, with Africa across the Strait."
chipsFiltros:
  - { label: "Cathedral", tag: "catedral" }
  - { label: "Old town", tag: "casco-antiguo" }
  - { label: "Beaches", tag: "playas" }
  - { label: "Jerez", tag: "jerez" }
  - { label: "Tarifa", tag: "tarifa" }
---

Cádiz is the southernmost provincial capital in mainland Spain —around 110,000 people— and, by tradition, the **oldest inhabited city in the West**, founded by the Phoenicians more than 3,000 years ago. Almost surrounded by sea on a narrow peninsula, it has a light and character all its own.

Its **old town** is compact and walkable: the **Cathedral** facing the Atlantic, the **watchtowers** such as Torre Tavira, the medieval **El Pópulo quarter** and the squares full of Cádiz's famous wit. And almost in the centre, **La Caleta beach**, the most characterful in the city.

Cádiz is also a perfect base for the province: half an hour away, **Jerez de la Frontera** (wine, flamenco and horses); further south, **Tarifa** and the great windswept beaches of the **Costa de la Luz**, with Africa across the Strait. The city is seen well in **one or two days**.

The best time to visit is **spring and autumn**; in February, the **Carnival of Cádiz** is one of the most famous in the world, but it fills the city completely.
